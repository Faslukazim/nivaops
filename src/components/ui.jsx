...
// ─── VacateModal ─────────────────────────────────────────────────────────────
// Shared vacate/advance-notice dialog — used from Tenants tab, tenant profile,
// and Rooms tab so every "Vacate" action goes through the same date + deposit
// flow instead of each surface having its own (possibly immediate-only) path.

export function VacateModal({ tenant, onConfirm, onCancel, saving }) {
  ...
  // Rendered via portal into document.body — this modal can be triggered from
  // inside a swipe-to-reveal wrapper with a permanent CSS transform on it.
  // A transform on any ancestor traps position:fixed children inside that
  // ancestor's box instead of the viewport, so without the portal this modal
