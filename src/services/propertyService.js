...
export async function fetchProperties() {
  const { data, error } = await supabase
    .from('properties')
    .select('id, name, total_beds, address, status, is_demo, upi_id, organization_id, city, is_listed, locality, listing_description, cover_photo_url, gender_preference, amenities, whatsapp_number')
    .eq('status', 'active')
    .order('name');
